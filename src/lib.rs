...
impl MIDIChannel {
    ...
    // pub fn pitch_bend_sensitivity(&mut self, sensitivity: u8) {
    //     self.pitch_bend_sensitivity = sensitivity
    // }
}
...
impl MPEState {
    ...
    pub fn member_channels(&self, zone: &Zone) -> Option<&[Channel]> {
        match self.channels[zone.manager_index()] {
            Channel::Manager {
                member_channels, ..
            } => {
                let manager_offset = 1;
                Some(self.zone_slice(
                    zone,
    ...
    pub fn zone_channels(&self, zone: &Zone) -> Option<&[Channel]> {
        match self.channels[zone.manager_index()] {
            Channel::Manager {
                ...
                let manager_offset = 1;
                Some(self.zone_slice(zone, 0..(member_channels as usize + manager_offset)))
            }
            _ => None,
        }
    }
    pub fn zone_channels_mut(&mut self, zone: &Zone) -> Option<&mut [Channel]> {
        match self.channels[zone.manager_index()] {
            Channel::Manager {
                ...
                let manager_offset = 1;
                Some(self.zone_slice_mut(zone, 0..(member_channels as usize + manager_offset)))
            }
            _ => None,
        }
    }
